...
2. **Secrets** — `appsettings.Local.json` is gitignored. Never commit tokens.
3. **ApexCharts** — each chart needs its own `ApexChartOptions<T>` instance. Shared instances cause blank charts.
4. **Timezones** — Oura timestamps have local offsets. Call `.ToUniversalTime()` before assigning to EF entities.
5. **Custom metrics** — `NightMetrics.cs` is a pure static method (no DI, no DB). Keep it that way.
6. **Upsert** — all sync operations upsert on `(UserId, Day)`. Safe to re-run.
...
| `/sync` | `Sync.razor` — sync status + trigger |
